...
client.on("messageCreate", async (message) => {
  ...
  if (nprefix.includes(client.user.id)) {
    message.reply(`**Um alle meine Commands zu sehen **\`/help\``);
  }
      ...
      return message.reply({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `** ❌ Du hast nicht die ${command.userPermissions} Rechte um diesen Command auszuführen.. **`
            ),
        ],
      });
      ...
      return message.reply({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `** ❌ Ich habe nicht die ${command.botPermissions} Rechte um diesen Command auszuführen.. **`
            ),
        ],
      });
